...
async function checkSubscription() {
  ...
  if (subscription) {
    ...
    const response = await fetch("/api/thrust/webpush/isSubscribed", {
      method: 'POST',
...
async function init() {
  fetch('/api/thrust/webpush/publicSigningKey')
...
async function unsubscribe() {
  ...
  if (subscription) {
    ...
    if (successful) {
      ...
      await fetch("/api/thrust/webpush/unsubscribe", {
        method: 'POST',
        body: JSON.stringify({endpoint: subscription.endpoint}),
        headers: {
          "content-type": "application/json"
...
async function subscribe() {
  ...
  await fetch("/api/thrust/webpush/subscribe", {
    method: 'POST',
